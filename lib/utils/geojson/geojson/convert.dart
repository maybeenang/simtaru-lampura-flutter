...
void convertFeature(List featureCollection, geojson, options, index) {
  ...
  const geomString = "geometry";
  const propString = "properties";

  if (geomString.isEmpty) return;
  ...
  if (featureType == FeatureType.Point) {
    ...
    if (options.lineMetrics != null && options.lineMetrics) {
      ...
      for (var line in coords) {
        ...
        featureCollection.add(createFeature(
            id, FeatureType.LineString, geometry, geojson[propString]));
      }
      return;
    } else {
    ...
    for (final singleGeometry in geojson[geomString]
        [Feature.keyLookupString("geometries", shortKeys)]) {
      convertFeature(
          featureCollection,
          {
            'id': id, // to do
  ...
  featureCollection
      .add(createFeature(id, featureType, geometry, geojson[propString]));
